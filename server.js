...
app.post('/api/spin', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    // Insert unique per device/prize; ignore if already owned
    db.prepare('INSERT OR IGNORE INTO collections(device_id, prize_id) VALUES (?,?)').run(device, pick.id);
  });
  tx();
  // whether newly added or duplicate
  const owned = db.prepare('SELECT 1 FROM collections WHERE device_id=? AND prize_id=?').get(device, pick.id) ? true : false;
  res.json({
    ok: true,
    prize: {
      title: pick.title,
    ...
    addedToCollection: owned // true if present (after insert-or-ignore it's always true)
  });
});
